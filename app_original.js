...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const getSegmentColor = (name) => {
        ...
        if (lower.includes('sem segmenta├º├úo')) return '#9CA3AF';
        if (lower.includes('clientes fi├®is') || lower.includes('clientes fieis')) return '#2563EB';
        if (lower.includes('potencial fiel')) return '#93C5FD';
        if (lower.includes('campe├úo') || lower.includes('campeao')) return '#00A650';
        ...
        if (lower.includes('precisa de aten├º├úo') || lower.includes('atencao')) return '#F472B6';
        if (lower.includes('clientes promissores')) return '#FBCFE8';
        if (lower.includes('n├úo perder') || lower.includes('nao perder')) return '#60A5FA';
    ...
    const loadBaseTotal = async () => {
        try {
            ...
            if (periodPickerEl) {
                ...
                allBtn.textContent = 'Limpar Per├¡odo (Todo Hist├│rico)';
                ...
                function updatePeriodUI() {
                    ...
                    if (mainPeriodTextEl) {
                        if (window.selectedYears.size === 0 && window.selectedMonths.size === 0) {
                            mainPeriodTextEl.textContent = 'Todo Hist├│rico';
                        } else {
            ...
            window.updateBaseTotalKPI = () => {
                ...
                if (delta60dEl) {
                    if (t30 > 0) {
                        ...
                        delta60dEl.innerHTML = `<span style="color: ${pct >= 0 ? '#10B981' : '#EF4444'}; font-weight: bold;">${pct > 0 ? 'Ôåæ' : 'Ôåô'} +${Math.abs(pct).toFixed(0)}%</span> vs 30 dias`;
                    } else {
                ...
                if (delta90dEl) {
                    if (t60 > 0) {
                        ...
                        delta90dEl.innerHTML = `<span style="color: ${pct >= 0 ? '#10B981' : '#EF4444'}; font-weight: bold;">${pct > 0 ? 'Ôåæ' : 'Ôåô'} +${Math.abs(pct).toFixed(0)}%</span> vs 60 dias`;
                    } else {
                ...
                if (total > 0) {
                    ...
                    // --- Progress├úo da Base Logic (Insight) ---
            ...
            if (statusCanvas) {
                ...
                const baseColors = labels.map((label, index) => {
                    ...
                    if (l.includes('n├úo ativado') || l === 'nao ativado') return '#9CA3AF'; // Cinza
                    if (l.includes('churn')) return '#DC2626'; // Vermelho
                    if (l.includes('inativo')) return '#F97316'; // Laranja
                    if (l.includes('dormente') || l.includes('risco') || l.includes('aten├º├úo')) return '#1E3A8A'; // Azul
    ...
    const updateEvolutionCharts = () => {
        ...
        // Data for Fi├®is, Base Total, and Ativos (from baseTotalData)
    ...
    const loadActiveClients = async () => {
        try {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                if (canal === 'LOJA_F') formattedCanal = 'Loja f├¡sica';
        ...
        try {
            ...
            if (perfilTbody && perfilTfoot && totalGender > 0) {
                ...
                if (summaryText) {
                    ...
                    summaryText.innerHTML = `Maior concentra├º├úo de clientes na faixa de ${biggestAge} anos (${pctBiggest}% da base analisada).<br>
                    Homens representam ${overallPctM.split(',')[0]}% do total em todas as faixas et├írias.`;
                }
            }
    ...
    const loadFrequenciaTicket = async () => {
        try {
            ...
            const getChannelName = (c) => {
                if (c === 'LOJA_F') return 'Loja F├¡sica';
            ...
            const renderFrequenciaDashboard = (selectedCanal) => {
                ...
                if (channelData.length >= 2) {
                    ...
                    const renderKpi = (idVal, idDelta, curr, prevVal, isPct, isInverse, decimals, prefix) => {
                        ...
                        el.innerHTML = `${arrow} ${deltaStr} <span style="color:#6B7280; font-weight:500;">vs m├¬s anterior</span>`;
                    };
                    ...
                    if (deltaDias > 0) {
                        ...
                        deltaDiasEl.innerHTML = `&#9650; +${deltaDias.toFixed(1).replace('.', ',')} dias <span style="color:#6B7280; font-weight:500;">vs m├¬s anterior</span>`;
                    } else if (deltaDias < 0) {
                        deltaDiasEl.className = 'freq-kpi-delta positive';
                        deltaDiasEl.innerHTML = `&#9660; ${deltaDias.toFixed(1).replace('.', ',')} dias <span style="color:#6B7280; font-weight:500;">vs m├¬s anterior</span>`;
                    } else {
                        deltaDiasEl.className = 'freq-kpi-delta subtext';
                        deltaDiasEl.innerHTML = `- 0 dias <span style="color:#6B7280; font-weight:500;">vs m├¬s anterior</span>`;
                    }
                }
            ...
            channelsToCompare.forEach((c, idx) => {
                ...
                const getBadge = (val, refVal, isInverse) => {
                    ...
                    if (ratio > 1) {
                        text = `${ratio.toFixed(2).replace('.', ',')}x maior que Loja F├¡sica`;
                        badgeClass = isInverse ? 'red' : 'green';
                    } else if (ratio < 1) {
                        text = `${ratio.toFixed(2).replace('.', ',')}x da Loja F├¡sica`;
                    ...
                    <td>
                        <div style="display:flex; align-items:center; gap:12px;">
                            ...
                            <div>
                                ...
                                <div class="comp-channel-sub">${c === 'LOJA_F' ? '(refer├¬ncia)' : (c === 'CANAIS_VIRTUAIS' ? '(APP, SITE, iFood)' : '')}</div>
                            </div>
                        </div>
                    </td>
                    <td>
    ...
    const loadCategorias = async () => {
        try {
            ...
            // Populate filters
            const generos = [...new Set(categoriasData.map(d => d.sexo).filter(s => s && s !== 'N├úo Informado'))].sort();
